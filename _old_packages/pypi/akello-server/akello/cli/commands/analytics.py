...
def get_users_in_registry(db_table, registry_id):
    ...
    response = db_table.scan(FilterExpression=Key('partition_key').eq(partition_key))
    return response['Items']


def get_user(db_table, user_id):
    partition_key = f'user:{user_id}'
    response = db_table.get_item(Key={'partition_key': partition_key, 'sort_key': 'profile'})
...
def scan_patients(cognito, userpool_id, db_table, registry_id):
    ...
    for user in users:
        ...
        try:
            user_profile = cognito.admin_get_user(UserPoolId=userpool_id, Username=user_object['email'])
        ...
        user_emails.append({'email': user_object['email'], 'given_name': given_name, 'family_name': family_name,
    ...
    if len(patient_created_stats.items()) == 0:
        for user_email in user_emails:
            print(
                f"{registry_id},{user_email['given_name']},{user_email['family_name']},{user_email['email']},{user_email['last_login']},,")

    else:
        for key, value in patient_created_stats.items():
            for user_email in user_emails:
                print(
                    f"{registry_id},{user_email['given_name']},{user_email['family_name']},{user_email['email']},{user_email['last_login']}, {key}, {value}")
