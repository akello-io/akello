    ...
    return (
      <AppShell.Header className='flex justify-between px-4'>
        <div className='flex flex-row space-x-3'>
          ...
          <div className='text-xl font-bold my-auto'>Akello.io</div>
        </div>
        <div className='flex flex-row my-auto space-x-6'>
          {akello.getSelectedRegistry() && (
            <div
              className='btn btn-neutral btn-sm rounded-xl my-auto'
              onClick={() => {
                navigate(`/registry/${akello.getSelectedRegistry()}/patient-referral`);
              }}
            >
              Add Patient
            </div>
          ...
          {!akello.getSelectedRegistry() && (
            <button className='btn btn-neutral btn-sm rounded-xl my-auto' onClick={() => { navigate('/create-registry'); }}>
              Create Registry
            </button>
          )}
